...
import argparse
from strands.hooks import BeforeToolCallEvent,AfterToolCallEvent, BeforeModelCallEvent, AfterModelCallEvent
...
def tool_output_process(event)-> None:
    event_type_str = str(type(event))
    split1 = event_type_str.split('\'')
...
with streamable_http_mcp_client:
    ...
    #print(">>> Before Model")    
    agent.hooks.add_callback(BeforeModelCallEvent, 
                             tool_output_process)
    #print(">>> After Model")    
    agent.hooks.add_callback(AfterModelCallEvent, 
                             tool_output_process)
