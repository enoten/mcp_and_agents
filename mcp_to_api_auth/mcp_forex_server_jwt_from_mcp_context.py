...
- Outbound API calls forward the same JWT from MCP context (no token generation)
- Requires MCP_JWT_SECRET == FAPI_JWT_SECRET so the client's token is valid for the API
"""
import base64
import hashlib
import hmac
...
def _get_token_from_mcp_context() -> str:
    """Extract Bearer token from the current MCP request context."""
    headers = get_http_headers()
    auth_header = headers.get("authorization")
    if not auth_header or not auth_header.lower().startswith("bearer "):
        raise ToolError("Unauthorized: Missing Authorization header in context")
...
def cross_currency_rate(base: str, target: str):
    ...
    Requires: Authorization: Bearer <JWT> header. The same token is forwarded to the API
    (extracted from MCP context, not generated).
    ...
    """
    token = _get_token_from_mcp_context()
...
if __name__ == "__main__":
    ...
    # Token is forwarded from MCP context; API must use same secret (MCP_JWT_SECRET == FAPI_JWT_SECRET)
